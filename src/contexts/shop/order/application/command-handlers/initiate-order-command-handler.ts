...
export class InitiateOrderCommandHandler implements CommandHandler<InitiateOrderCommand> {
    ...
    async handle(command: InitiateOrderCommand): Promise<void> {
        ...
        const address = new OrderAddress(new OrderStreet(command.address.street), new OrderCity(command.address.city), command.address.number);
