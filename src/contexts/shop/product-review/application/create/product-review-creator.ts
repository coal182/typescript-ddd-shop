...
export class ProductReviewCreator {
    ...
    async run(params: {id: ProductReviewId; productId: ProductId; userId: UserId; rating: ProductReviewRating; comment: ProductReviewComment}): Promise<void> {
        await this.productFinder.run(params.productId);

        const productReview = ProductReview.create(params.id, params.productId, params.userId, params.rating, params.comment);
