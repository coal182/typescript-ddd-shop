...
export class StoreBackendApp {
  ...
  async stop() {
    const rabbitMQConnection = this.container.get<RabbitMqConnection>('Store.Shared.RabbitMQConnection');
  ...
  private async configureEventBus() {
    ...
    const eventBus = this.container.get<EventBus>('Store.Shared.domain.EventBus');
    const rabbitMQConnection = this.container.get<RabbitMqConnection>('Store.Shared.RabbitMQConnection');
