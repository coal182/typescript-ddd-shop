...
import {
  ...
  createProductReviewCodec,
} from 'src/contexts/shop/product-review/application/commands/create-product-review';
...
export class ProductReviewPostController {
  ...
  async run(req: Request, res: Response) {
    try {
      const { id, productId, userId, rating, comment } = ParamsParser.parse<Primitives<CreateProductReviewCommand>>(
        req.body,
        createProductReviewCodec
      );
