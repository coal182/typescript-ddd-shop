...
import {CreateProductCommand, createProductCodec} from 'src/contexts/backoffice/product/application/commands/create-product';
...
export class ProductPostController {
    ...
    async run(req: Request, res: Response): Promise<void> {
        try {
            const {id, name, description, images, price, brand, category, ean} = ParamsParser.parse<Primitives<CreateProductCommand>>(
                req.body,
                createProductCodec,
            );
